...
print("Model Name: Deeper Insights into Heterophilic Graphs")
...
def train(train_loader, test_loader, model, opti_train, train_iter, model_path, device):
    ...
    for i in range(train_iter):
        ...
        # edge_ratio_list.append(avg_edge_ratio.item())
        # dir_energy_list.append(avg_dir_energy)

        train_acc, val_acc, test_acc = test(model, test_loader)
        
        # print(f'Avg Edge Ratio: {avg_edge_ratio:.4f} || Avg Prob: {avg_prob.item():.4f} || Avg Dir Energy: {avg_dir_energy}')
